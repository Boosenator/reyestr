...
class DetailPanel(tk.Frame):
    ...
    def _collect_metadata(self):
        ...
        is_ctrl = int(self._fields["is_controlled_var"].get())
        deadline = self._fields["deadline"].get_date().isoformat() if is_ctrl else None
        desc = self._fields["description"].get("1.0", "end-1c").strip()
        return (status, doc_type, doc_number, doc_date, sender, tags, is_ctrl, deadline, desc)

    def _update_metadata(self, did):
        vals = self._collect_metadata()
        db.execute(
            "UPDATE documents SET status=?,doc_type=?,doc_number=?,doc_date=?,sender=?,tags=?,is_controlled=?,deadline=?,description=? WHERE id=?",
            (*vals, did),
        )

    def _save_single(self, did):
        row = db.query("SELECT filename, filepath FROM documents WHERE id=?", (did,))
        if not row:
        ...
        if base:
            ...
            new_name = base + ext
            if os.path.basename(old_path) != new_name and os.path.exists(old_path):
        ...
        self._update_metadata(did)
        save_document_numbers(did, get_document_numbers(did))
